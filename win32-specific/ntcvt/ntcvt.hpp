...
public:
  ...
  _Elem* setnbuf(int len)
  {
    this->reserve(len);
#if _MSC_VER > 1900 // VS2017 or later
    auto _Myval             = (std::_Compressed_pair<_Alty, _Scary_val>*)this;
...
inline _StringContType wcbs2a(const wchar_t* wcb, int len, UINT cp = CP_UTF8)
{
  if (len == -1)
    len = lstrlenW(wcb);
...
template <typename _StringContType>
inline _StringContType mcbs2w(const char* mcb, int len, UINT cp = CP_UTF8)
{
  if (len == -1)
    len = lstrlenA(mcb);
  _StringContType buffer;
...
#if _HAS_CXX17
inline std::string from_chars(const std::wstring_view& wcb, UINT cp = CP_UTF8)
{
  return wcbs2a<std::string>(wcb.data(), wcb.length(), cp);
}

inline std::wstring from_chars(const std::string_view& mcb, UINT cp = CP_UTF8)
{
  return mcbs2w<std::wstring>(mcb.data(), mcb.length(), cp);
}
#else
inline std::string from_chars(const std::wstring& wcb, UINT cp = CP_UTF8)
{
  return wcbs2a<std::string>(wcb.c_str(), wcb.length(), cp);
}

inline std::wstring from_chars(const std::string& mcb, UINT cp = CP_UTF8)
{
  return mcbs2w<std::wstring>(mcb.c_str(), mcb.length(), cp);
}
#endif

inline std::string from_chars(const wchar_t* str, UINT cp = CP_UTF8)
{
  return wcbs2a<std::string>(str, -1, cp);
}

inline std::wstring from_chars(const char* str, UINT cp = CP_UTF8)
{
...
#if defined(_AFX)
inline std::string from_chars(const CStringW& wcb, UINT cp = CP_UTF8)
{
...
{
#  if _HAS_CXX17
inline CStringW from_chars(std::string_view mcb, UINT cp = CP_UTF8)
{
  return mcbs2w<CStringW>(mcb.data(), mcb.length(), cp);
}
#  else
inline CStringW from_chars(const char* str, UINT cp = CP_UTF8)
{
  return mcbs2w<CStringW>(str, -1, cp);
}
inline CStringW from_chars(const std::string& mcb, UINT cp = CP_UTF8)
{
